...
{
    public class ReturnDocument : IDocument
    {
        public string DocumentNumber { get; set; }
        public DocumentType DocumentType { get; set; }

        public ReturnDocument(string documentNumber, DocumentType documentType)
        {
        ...
        public DocumentData? GetDocumentData()
        {
            ...
            using (StrsTuraArchiveNewContext context = new StrsTuraArchiveNewContext())
            {
                try
                {
                    var data = context.MetaReturorders.AsNoTracking().OrderByDescending(y => y.BlobInfoCreationDateTime).AsNoTracking()
